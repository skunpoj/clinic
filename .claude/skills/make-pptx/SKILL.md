...
Trigger this skill whenever the user asks to create, edit, read, or regenerate any `.pptx` presentation — including the Clinical Note Retrieval System deck.
...
| Regenerate project deck | Run `python3 scripts/make_pptx.py` |

---

## Project Shortcut — Regenerate the Clinical Deck

When the user runs `/make-pptx` with no other instructions, regenerate the project presentation:

```bash
pip install python-pptx markitdown --quiet
python3 scripts/make_pptx.py
```

Then:
1. Run Content QA (see QA section below)
2. Deliver `Clinical_Note_Retrieval_System.pptx` to the user
3. Commit and push:
   ```bash
...
The deck covers: Problem Statement · Architecture · Embedding Model Selection · Retrieval Pipeline · LLM Summarisation · REST API · Evaluation · Semantic-vs-Keyword Proof · Fine-tuning Roadmap.
...
> **For the clinical deck**: the current palette is Navy `0A294A` + Teal `007A87` + White. Matches "Teal Trust" above.
...
Convert slides to images, then inspect with this prompt:
...
- Uneven gaps (large empty area one place, cramped another)
...
- `pip install Pillow python-pptx` — thumbnail grids + project generator
